...
def prepare_dataset(description, raw_data):
    dataset = {}
    years_list = [year.strip() for year in description[1]]
...
def get_country_data(dataset, country_iso):
    ...
    data = [(entry.get('year'), entry.get('coverage')) for entry in dataset.get(country)]
    return {country: data}

def get_year_data(dataset, year):
    country_coverage = [(country, data[1]) for country in dataset for data in get_country_data(dataset, country)[country] if year in data]
    return({year: country_coverage})

def get_average_for_country(dataset, country_iso):
    country = get_country_name(country_iso)
    coverage_values = [data[1] for data in get_country_data(dataset, country_iso)[country]]
    return(mean(coverage_values))

def get_average_for_year(dataset, year):
    coverage_values = [data[1] for data in get_year_data(dataset, year)[year]]
...
print('The average coverage for {} is: {:.2f}'.format(get_country_name(country_iso_for_average), get_average_for_country(dataset, country_iso_for_average)))

#Retrieve average for a specific year
year_for_average = '2016'
print('The average coverage for {} is: {:.2f}'.format(year_for_average, get_average_for_year(dataset, year_for_average)))
